const l2dSpineModule = (function () {
    ...
    class l2dSpine {
        ...
        slBoxRender(json) {
            const cselBox = document.querySelector('.cselContinerWrap');
            console.log(json);
            let slModels = [];
            for (let i in json) {
                for (let j of json[i]) {
                    slModels.push({
                        n: j.f,
                        d: [j.ds[client.langIndex],j.n[client.langIndex]],
                        etype: i,
                        p: j.f + '/' + j.m[0],
                        fsufix: j.m[1]
                    })
        ...
        init() {
            ...
            let stmsg = ['Click To Start Render.', 'Drag chrCard from bottom to here.'];
            ((stmsg) => {
